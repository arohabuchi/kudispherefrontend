...
export default function ImageUploadForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append("testImage", file); // must match `upload.single("testImage")`
    ...
    try {
      ...
      // ✅ POST to your backend image upload endpoint
      const res = await axios.post<UploadResponse>(
        `${BACKEND_BASEURL}/api/images`,
        formData,
        {
          headers: {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="number"
              value={gasfee}
              onChange={(e) => setGasfee(e.target.value ? Number(e.target.value) : "")}
              className="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-700 px-3 py-2 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
