...
const PaperAirplaneIcon = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.491 12.001c-1.287 4.098-4.148 7.03-7.5 9.764" />
  </svg>
);
...
// const BankWithdrawalModal = ({ isOpen, onClose, withdrawalAmount, onSubmitWithdrawal }) => {
//   const [bankName, setBankName] = useState('');
//   const [accountNumber, setAccountNumber] = useState('');
//   const [accountHolderName, setAccountHolderName] = useState('');
//   const [isSubmitting, setIsSubmitting] = useState(false);
...
//         })
//         .catch(err => console.error("Error fetching admin bank details:", err));
//     }
//   }, [isOpen]);
...
//     onSubmitWithdrawal(payload, () => setIsSubmitting(false));
//   };

//   // calculate summary
//   const fee = withdrawalAmount * feePerUSDT;
//   const amountToReceive = (withdrawalAmount * usdtRate) - fee;
...
//         {/* Transaction Summary */}
//         <div className="bg-gray-100 dark:bg-gray-800 p-3 rounded-lg text-sm space-y-1">
//           <p><span className="font-semibold">Fee per USDT:</span> {feePerUSDT}</p>
//           <p><span className="font-semibold">USDT Rate:</span> {usdtRate}</p>
//           <p><span className="font-semibold">Conversion Summary:</span> {withdrawalAmount} × {usdtRate} - ({withdrawalAmount} × {feePerUSDT})</p>
//           <p className="font-bold text-green-600">Amount to Receive: ${amountToReceive.toFixed(2)}</p>
//         </div>
...
//             id="bankName"
//             type="text"
//             className="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-600"
//             value={bankName}
//             onChange={(e) => setBankName(e.target.value)}
//             required
//           />
...
//           type="submit" 
//           disabled={isSubmitting}
//           className="bg-red-600 hover:bg-red-700 w-full text-md py-3 rounded-xl shadow-md mt-6"
//         >
//           {isSubmitting ? 'Processing...' : 'Confirm Bank Transfer'}
//         </Button>
//       </form>
...
// const CryptoWithdrawalModal = ({ isOpen, onClose, withdrawalAmount, onSubmitWithdrawal }) => {
//   const [walletAddress, setWalletAddress] = useState('');
//   const [network, setNetwork] = useState('');
//   const [isSubmitting, setIsSubmitting] = useState(false);
...
//   const coinType = "USDT"; // Non-editable coin type
...
//   return (
//     <Modal isOpen={isOpen} onClose={onClose} title="Transfer to Crypto Wallet">
//       <form className="space-y-4" onSubmit={handleSubmit}>
...
//         {/* Coin Type (Non-editable) */}
//         <div>
//           <label htmlFor="coinType" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
//             Coin Type
//           </label>
//           <input
//             id="coinType"
//             type="text"
//             className="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white px-3 py-2 cursor-not-allowed bg-gray-100 dark:bg-gray-700 font-semibold"
//             value={coinType}
//             disabled
//           />
//         </div>
...
//         {/* Warning */}
//         <div className="p-3 text-xs bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 rounded-lg text-yellow-800 dark:text-yellow-300">
//           <p className="font-semibold">Important Warning:</p>
//           <p>We are not responsible for funds transferred to a wrong wallet address or network provided by you. Please double-check your details.</p>
//         </div>
...
//           className="bg-red-600 hover:bg-red-700 w-full text-md py-3 rounded-xl shadow-md mt-6"
//         >
//           {isSubmitting ? 'Processing...' : 'Confirm Crypto Transfer'}
//         </Button>
//       </form>
//     </Modal>
//   );
...
//   const handleDeposit = async () => {
//     try {
//       // NOTE: Using document.execCommand('copy') is generally recommended in iFrame environments
//       // but the original code used navigator.clipboard.writeText(). Leaving it for now.
//       const token = localStorage.getItem("token");
...
//                   onClick={() => {
//                     if (coinData?.name) {
//                       // NOTE: Using document.execCommand('copy') is safer in iFrame environments
//                       document.execCommand('copy'); 
...
// // Bank Payment Modal (New/Modified)
// const BankPaymentModal = ({ isOpen, onClose, amountGBP, bankDetails, onSubmitDeposit, isSubmitting }) => {
//   if (!isOpen || !bankDetails) return null;
...
//   const NGN_RATE = 1629; 
//   const depositAmountUSD = Number(amountGBP); 
//   // feePerUSDT is the percentage fee value from the backend
//   const feePerUSDT = bankDetails.feePerUSDT || 0; 
...
//     // Construct the payload to match the backend model and endpoint requirements
//     const payload = {
//       amount: depositAmountUSD, // amount (in USD/USDT)
//       bankName: bankDetails.bankName,
//       accountNumber: bankDetails.accountNumber,
...
//   const [isBankCheckoutOpen, setIsBankCheckoutOpen] = useState(false);
//   const [depositAmount, setDepositAmount] = useState("");
//   const [coinType, setCoinType] = useState("USDT");
//   const [depositMethod, setDepositMethod] = useState("bank"); // Defaulting to bank as per initial UI
...
//   // New state for Admin Bank Details
//   const [adminBankDetails, setAdminBankDetails] = useState(null);
//   const [isBankLoading, setIsBankLoading] = useState(false);
//   const [isSubmitting, setIsSubmitting] = useState(false); // New state for bank deposit submission
...
//   // New function to fetch Admin Bank Details
//   const fetchAdminBankDetails = async () => {
//     // If details are already fetched and not currently loading, skip the fetch
//     if (adminBankDetails && !isBankLoading) return true; 
...
//         const data = await res.json();
//         // Assuming the response is { bankDetails: { ... } } based on the Mongoose model
//         // The bank details should contain { bankName, accountNumber, accountHolderName, feePerUSDT }
//         setAdminBankDetails(data.bankDetails); 
//         return true;
//     } catch (err) {
//         console.error("Error fetching admin bank details:", err.message);
//         // IMPORTANT: Replace alert with a custom modal UI in a real app
//         alert("Failed to load bank details for deposit. Please try again.");
//         return false;
//     } finally {
//         setIsBankLoading(false);
//     }
...
//   /**
//    * Submits the bank deposit transaction to the backend.
//    * @param {Object} payload - The transaction data including amount and bank details.
//    */
//   const handleSubmitBankDeposit = async (payload) => {
//     if (!userId) {
...
//       // IMPORTANT: Replace alert with a custom modal UI in a real app
//       alert("Bank deposit request submitted successfully! Status: pending review.");
...
//       console.error("Error submitting bank deposit:", error.message);
//       // IMPORTANT: Replace alert with a custom modal UI in a real app
//       alert(`Error submitting bank deposit: ${error.message}`);
//     } finally {
//       setIsSubmitting(false);
//     }
//   };
...
//   // Fetch coin data when coinType changes (only relevant for crypto)
//   useEffect(() => {
//     if (!coinType || depositMethod === "bank") return;
...
//   // Handle Deposit Now click logic
//   const handleDepositNow = async () => {
//     setIsFundModalOpen(false); // Close fund modal

//     if (depositMethod === "bank") {
//         // Fetch bank details before opening the modal
//         const success = await fetchAdminBankDetails();
//         if (success) {
//           // If bank details are available, open the bank checkout modal
//           setIsBankCheckoutOpen(true);
//         }
//     } else {
//         // Otherwise, open the crypto checkout modal
//         setIsCheckoutOpen(true);
//     }
//   };
...
//           <h4 className="text-gray-500 dark:text-gray-400 text-sm">Available Balance</h4>
//           <div className="flex justify-between items-center mt-3">
//             <p className="text-2xl font-bold text-gray-800 dark:text-white">
//               {loading ? "Loading..." : amount.toLocaleString()}
//             </p>
//             <Badge color="success" onClick={() => setIsFundModalOpen(true)}>
//               <ArrowDownIcon className="size-4" />
//               Fund Account
//             </Badge>
//           </div>
...
//         {/* Payout */}
//         <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 md:p-6">
//           <h4 className="text-gray-500 dark:text-gray-400 text-sm">Total Payout</h4>
//           <div className="flex justify-between items-center mt-3">
//             <p className="text-2xl font-bold text-gray-800 dark:text-white">
//               {loading ? "Loading..." : payout.toLocaleString()}
//             </p>
//             <Badge color="error" onClick={() => setIsTransferModalOpen(true)}>
//               <ArrowUpIcon className="size-4" />
//               Transfer Funds
//             </Badge>
//           </div>
...
//               />
//               <select
//                 className="w-28 rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-600"
//                 value={coinType}
//                 onChange={(e) => setCoinType(e.target.value)}
//                 // DISABLE DROPDOWN WHEN DEPOSIT METHOD IS 'bank'
//                 disabled={loading || depositMethod === "bank"} 
//               >
//                 <option value="">{loading ? "Loading..." : "Select Coin"}</option>
//                 {coinTypes.map((type, index) => (
//                   <option key={index} value={type.toUpperCase()}>
...
//                 <span>Total</span>
//                 <span>${depositAmount ? (Number(depositAmount) + 0.64).toFixed(2) : "0.00"}</span>
//               </div>
//             </div>
//           </div>
...
//           <Button
//             type="button"
//             // If bank method is selected, disable if bank details are currently loading
//             disabled={!depositAmount || Number(depositAmount) <= 0 || (depositMethod === 'bank' && isBankLoading)} 
//             onClick={handleDepositNow}
//             className="bg-green-600 hover:bg-green-700 w-full text-md py-3 rounded-xl shadow-md"
//           >
//             {depositMethod === 'bank' && isBankLoading ? 'Loading Bank Details...' : 'Deposit Now'}
...
//     {/* NEW: Withdrawal Method Select Field */}
//     <div>
//       <label htmlFor="withdrawal-method" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
//         Withdrawal Method
//       </label>
...
//       >
//         <option value="bank">Withdraw to bank</option>
//         <option value="usdt">Withdraw to Usdt</option>
//       </select>
//     </div>
    
//     {/* Existing: Amount Input Field */}
//     <div>
//       <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Amount to transfer</label>
//       <input
//         type="number"
//         className="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-600 transition duration-150 ease-in-out"
//         value={depositAmount}
...
//     {/* Submit Button */}
//     <Button type="submit" className="bg-red-600 hover:bg-red-700 w-full text-md py-3 rounded-xl shadow-md">
//       Transfer Now
//     </Button>
//   </form>
// </Modal>
...
//       {/* Bank Payment Modal - Now passing bankDetails, onSubmitDeposit, and isSubmitting */}
//       <BankPaymentModal
//         isOpen={isBankCheckoutOpen}
//         onClose={() => setIsBankCheckoutOpen(false)}
//         amountGBP={depositAmount}
...
// // // Bank Payment Modal (New/Modified)
// // const BankPaymentModal = ({ isOpen, onClose, amountGBP, bankDetails }) => {
// //   if (!isOpen || !bankDetails) return null;

// //   // Constants and Calculations (assuming amountGBP is the deposit amount in USD)
// //   const NGN_RATE = 1629; 
// //   const depositAmountUSD = Number(amountGBP); 
// //   const feePercent = bankDetails.feePerUSDT || 0;

// //   // Calculate fees and final received amount
// //   const feeUSD = (depositAmountUSD * (feePercent / 100)); // Fee calculated as percentage of deposit amount
// //   const amountToReceive = (depositAmountUSD - feeUSD);
// //   const totalAmountWithFee = (depositAmountUSD + 0.64); // Re-using the fixed fee from the main modal for consistency
// //   const amountToReceiveNGN = (amountToReceive * NGN_RATE).toLocaleString('en-US', { maximumFractionDigits: 0 });
...
// //           <div className="flex justify-between">
// //             <span className="font-medium text-gray-700">Fee per USDT</span>
// //             <span className="font-semibold text-red-600">{feePercent}%</span>
// //           </div>
// //         </div>
...
// //             </div>
// //             <div className="flex justify-between text-sm text-gray-600">
// //                 <span>Fee Deducted ({feePercent}%)</span>
// //                 <span>-${feeUSD.toFixed(2)} USD</span>
// //             </div>
// //             <div className="flex justify-between items-center mt-3 pt-2 border-t font-bold text-lg text-gray-800">
// //                 <span>You will Receive</span>
...
// //         <div className="mt-6">
// //           <button 
// //             // This button would typically submit the transaction to the backend after the user confirms the bank transfer
...
// //       {/* Bank Payment Modal - Now passing bankDetails */}
// //       <BankPaymentModal
// //         isOpen={isBankCheckoutOpen}
// //         onClose={() => setIsBankCheckoutOpen(false)}
// //         amountGBP={depositAmount}
...
// // //   const btcRate = 42142.11; // example static rate
// // //   const btcAmount = amountGBP ? (amountGBP / btcRate).toFixed(8) : "0";
...
// // //               <p className="text-xl font-bold">{amountGBP || 0} GBP</p>
// // //               <p className="mt-2 text-sm">Amount</p>
// // //               <p className="font-mono">{btcAmount} BTC</p>
// // //               <button
// // //                 onClick={handleDeposit}
// // //                 className="mt-4 w-full bg-purple-600 text-white rounded-lg py-2 font-medium"
// // //               >
// // //                 Open in Wallet
// // //               </button>
// // //             </div>
...
// // // // Bank Payment Modal (New)
// // // const BankPaymentModal = ({ isOpen, onClose, amountGBP }) => {
// // //   if (!isOpen) return null;
...
// // //           <span className="text-gray-600 text-sm">${amountGBP} USD</span>
// // //         </div>

// // //         {/* Summary */}
// // //         <h3 className="mt-4 mb-2 font-semibold text-gray-800">SUMMARY</h3>
...
// // //             <p className="text-xs text-gray-500">$1=NGN 1629</p>
// // //           </div>
// // //         </div>
...
// // //               $9
// // //             </p>
// // //             <p className="text-xs text-gray-500">NGN 9000</p>
// // //           </div>
// // //         </div>

// // //         {/* Slide Button */}
// // //         <div className="mt-6">
// // //           <button className="w-full bg-blue-600 text-white py-3 rounded-xl font-semibold shadow-md">
// // //             Slide to Send →
// // //           </button>
// // //         </div>
// // //       </div>
...
// // //   // Fetch coin data when coinType changes
// // //   useEffect(() => {
// // //     if (!coinType) return;
